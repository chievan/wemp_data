...
4. **投研助手问答引擎融合 (Assistant Q&A Integration)**：
   *   对 **投研助手** 问答检索底座 `DolphinDBVectorStore` 进行了深度定制，默认激活相同公式的指数时间衰减，使日常 Chat 问答自动具备敏锐的时效把控。
   *   **智能历史回溯保护**：采用局部词义正则引擎，智能识别用户是否指定了具体的年份或历史关键字（例如包含 "2024"、"历史"、"回顾" 等）。一旦识别到用户主动查阅历史，算法会自动退化为标准语义搜索（即 $\lambda=0$），保证了历史复盘研究与最新市场追踪的精细兼容。
   *   对问答系统提示词（`KNOWLEDGE_SYSTEM_PROMPT`）动态注入服务器当前绝对日期锚点，彻底解除 AI 的“时间色盲”状态。
